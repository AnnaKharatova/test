	...
	function updateArticle() {
		...
		articles.each(function(){
			...
			if( articleTop > scrollTop) {
				articleSidebarLink.removeClass('read active');
			} else if( scrollTop >= articleTop && articleTop + articleHeight > scrollTop) {
				var dashoffsetValue = svgCircleLength*( 1 - (scrollTop - articleTop)/articleHeight);
				articleSidebarLink.addClass('active').removeClass('read').find('circle').attr({ 'stroke-dashoffset': dashoffsetValue });
				changeUrl(articleSidebarLink.attr('href'));
			} else {
				articleSidebarLink.removeClass('active').addClass('read');
			}
		});
		scrolling = false;
	}
